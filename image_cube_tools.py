...
def spectrumNoiseEstimate(spec, xaxis = None, baselineDeg = None, clip1=2, clip2=2.5, returnBaseline =  False):
...
def makeThresholdMask(spectrumSN, narrowSN=3, broadSN=1.5, growSN = None, 
                        segJoin = None, minLen = 1,allSeg=False,plot=False):
...
def detectEmissionLines_mask(spectrum, linLambda, lineLambdas, 
                                ...
                                segJoin = 0, minLen = 1, allSeg=False,
                                plot=False):
    ...
    for lineLambda in lineLambdas:
        ...
        maskFinal = makeThresholdMask(spectrumSN,
                                        narrowSN = narrowSN, broadSN=broadSN, growSN=growSN, 
                                        segJoin=segJoin, minLen = minLen, allSeg=allSeg,
                                        plot=plot)
        ...
        maskVel_all.append(maskVel)
        maskSpectrum_all.append(maskSpectrum)
        maskFinal_all.append(maskFinal)

    output = [specNoise_all,
            np.array([linLambda_all,maskSpectrum_all,maskFinal_all,maskVel_all],dtype='object')]
...
def measureMaskedLineMoments(maskInfo,moments = [0],coordType = ['vel'],noiseIter=None):
    nLines = len(maskInfo[0])
    nMoments = len(moments)

    if len(coordType) ==1 and nMoments > 1:
        coordType = [coordType[0]]*nMoments

    lineMoments_all = []
    names_all = []
    for ll in range(nLines):
        specNoise = maskInfo[0][ll]
        linLambda = maskInfo[1][0,ll]   
        spec = maskInfo[1][1,ll] 
        mask =  maskInfo[1][2,ll] 
        vel  = maskInfo[1][3,ll]

        coord = []
        for mm in range(nMoments):
            if coordType[mm] == 'spec':
                coord.append(linLambda)
            elif coordType[mm] == 'vel':
                coord.append(vel)


        lineMoments = measureLineMoments(spec, coord, specNoise, mask = mask, moments = moments,noiseIter = noiseIter)

        lineMoments_all.append([specNoise] + lineMoments)
        
    # names = [f"{lineNames[ll]}_mom{mm}" for mm in moments]+ \
    # [f"{lineNames[ll]}_mom{mm}_unc" for mm in moments])

    # print(np.array(lineMoments_all))
    # print(names_all)
    # lineMoments_all = Table(data = lineMoments_all, names=names_all)
    return np.array(lineMoments_all)

def measureLineMoments(spectrum, coordinate, specNoise, mask = None, moments = [0],noiseIter = None): 
    ...
    if np.any(mask==1):
        ...
        if len(coordinate) == len(spectrum):
            coordinate = [coordinate]*len(moments)

        coordArr = []
        for mm in range(len(moments)):
            if moments[mm] == 0:
                coord = np.full_like(coordinate[mm],np.median(np.abs(np.diff(coordinate[mm]))))
            else:
                coord = coordinate[mm]
        ...
        moms = []
        for mm in range(len(moments)):
            mom = ST.weighted_moment(coordArr[mm][mask.astype(bool)],weights=spectrum[mask.astype(bool)],moment=moments[mm])
            moms.extend([mom])
        ...
        if not isinstance(noiseArr,type(None)):
            ...
            for nn in range(noiseIter):
                ...
                momNoiseIters.append([ST.weighted_moment(coordArr[mm][mask.astype(bool)],
                                weights=(spectrum+noiseArr[:,nn])[mask.astype(bool)],
                                moment=moments[mm])
                                for mm in range(len(moments))])

            momNoiseIters = np.array(momNoiseIters)

            sigmaMom = np.array([ST.median_absolute_deviation(momNoiseIters[:,mm],Niter=5)[1] for mm in range(len(moments))])

        else:
            sigmaMom = [np.nan]*len(moments)
                
        moms.extend(sigmaMom)
    else:
...
def extractLineMeasurements(output, type='mask'):


    if type == 'mask':
        moments = np.zeros([len(output),len(output[0][2]),len(output[0][2][0])])
    elif type == 'fit':
        moments = np.zeros([len(output),len(output[0][2]),len(output[0][2][0])])
    ...
    nSpectra = len(output)
    if type == 'mask':
            nLines = len(output[0][1])
    elif type == 'fit':
            nLines = len(output[0][0])
    ...
    for ll in range(nLines):
        ...
        if type == 'mask':
            linLambda = output[0][ll][1][0][0]
            lineVel = output[0][ll][1][3][0]
        elif type == 'fit':

            linLambda = output[0][1][0][ll]
            lineVel = output[0][1][3][ll]

        lineSpectra = np.zeros([len(linLambda),nSpectra])
        maskSpectra = np.zeros([len(linLambda),nSpectra])
    
        for ss in range(nSpectra):

            if type == 'mask':
                moments[ss,ll,:] = np.array(output[ss][2][ll]) 
                lineSpectra[:,ss] = output[ss][ll][1][1][0]
                maskSpectra[:,ss] = output[ss][ll][1][2][0]

            elif type == 'fit':
                moments[ss,ll,:] = np.array(output[ss][2][ll]) 
                lineSpectra[:,ss] = output[ss][1][1][ll]
                maskSpectra[:,ss] = output[ss][1][2][ll]
        ...
        allSpectra.append([linLambda,lineSpectra,maskSpectra,lineVel])
...
def plotMoments(moments,names,imgShape,nameExt = ''):
    ...
    labels1  = ['M0: Jy/beam km/s']+[f'M{mm}: km/s' for mm in range(1,nMoments)]
    labels2  = ['S/N']+[f'unc. M{mm}: km/s' for mm in range(1,nMoments)]

 

    for ll in range(nLines):

        fig, ax = plt.subplots(2,nMoments)
        ...
        for mm in range(nMoments):
            ...
            if mm==0:
                vmin1=0
                vmax1=np.percentile(moments[:,ll,1+mm][np.isfinite(moments[:,ll,1])],99)

                vmin2=0
                vmax2 = np.percentile((moments[:,ll,1+mm]/moments[:,ll,1+nMoments +mm])[np.isfinite(moments[:,ll,1])],95)
            elif mm==1:
                med,MAD = ST.median_absolute_deviation(moments[:,ll,1+mm],Niter=10)
                moments[:,ll,1+mm] -= med
                vmin1 = -0.5*MAD
                vmax1 = 0.5*MAD

                vmin2 = 0
                vmax2 = np.percentile(moments[:,ll,1+nMoments +mm][np.isfinite(moments[:,ll,1])],90)
            elif mm == 2:   
                vmin1 = 0
                vmax1 = np.percentile(moments[:,ll,1+mm][np.isfinite(moments[:,ll,1])],90)

                vmin2 = 0
                vmax2 = np.percentile(moments[:,ll,1+mm+nMoments][np.isfinite(moments[:,ll,1])],95)

            else:
                vmin1 = np.percentile(moments[:,ll,1+mm][np.isfinite(moments[:,ll,1])],5)
                vmax1 = np.percentile(moments[:,ll,1+mm][np.isfinite(moments[:,ll,1])],95)
...
def saveSubcubes(spectra,imgShape,names = None,outname="./subcubes.fits"):
    ...
    if isinstance(names,type(None)):
        names = [f"line{ll}" for ll in range(nLines)]
